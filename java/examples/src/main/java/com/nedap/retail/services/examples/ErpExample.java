...
import com.nedap.retail.client.model.ErpStock;
import com.nedap.retail.client.model.ErpStockSummary;
...
public class ErpExample {
    ...
    public static void runExample(final ApiClient client) {
        ...
        try {
            ...
            final ErpStock erpStock = createErpStock(locationId);
            final String stockId = erpStockApi.createErpStock(erpStock).getId();
            System.out.println("Captured ERP stock id: " + stockId);

            // Retrieve ERP stock
            System.out.println(NEW_LINE + "Retrieving ERP stock...");
            final ErpStock retrievedStock = erpStockApi.retrieveErpStock(stockId, true);
            ...
            final ErpStockSummary summary = erpStockApi.retrieveErpStockSummary(stockId);
            final ErpStock stock = castStockSummaryToStock(summary);
            ...
            final List<ErpStockSummary> stocks = erpStockApi.listErpStocks(locationId,
    ...
    private static ErpStock createErpStock(final String locationId) {
        final ErpStock stock = new ErpStock();
    ...
    private static String printStock(final ErpStock stock) {
    ...
    private static ErpStock castStockSummaryToStock(final ErpStockSummary summary) {
        final ErpStock stock = new ErpStock();
        ...
        stock.status(ErpStock.StatusEnum.fromValue(summary.getStatus().toString()));
    ...
    private static String printStockSummaryList(final List<ErpStockSummary> stocks) {
        ...
        for (int i = 0; i < stocks.size(); i++) {
            ...
            final ErpStock stock = castStockSummaryToStock(stocks.get(i));
