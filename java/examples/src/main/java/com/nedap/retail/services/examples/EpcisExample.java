...
public class EpcisExample {
    ...
    public static void runExample(final Client client) {
        System.out.println(PrintHelper.NEW_LINE + "*** EPCIS API example ***");

        final String locationId = client.getSites().get(0).getId();

        try {
            // Capture EPCIS events
            System.out.println(PrintHelper.NEW_LINE + "Capturing EPCIS events...");
            ...
            // Query EPCIS events
            System.out.println(PrintHelper.NEW_LINE + "Quering EPCIS events...");
            final List<EpcisEvent> events = client.queryEpcisEvents(makeEpcisQueryParameters());
            System.out.println(printEpcisEvents(events));

            System.out.println(PrintHelper.NEW_LINE + "--- EPCIS API example finished ---");
    ...
    private static EpcisEvent createEpcisEvent1(final String locationId) {
        ...
        event.id = generateEpcisEventID(PrintHelper.ORGANIZATION_PREFIX, 1).toString();
    ...
    private static EpcisEvent createEpcisEvent2(final String locationId) {
        final ObjectEvent event = new ObjectEvent();
        event.id = generateEpcisEventID(PrintHelper.ORGANIZATION_PREFIX, 2).toString();
        event.eventTime = DateTime.now();
        event.action = Action.OBSERVE.action();
        event.disposition = Disposition.SELLABLE_ACCESSIBLE.disposition();
        event.bizLocation = locationId;
    ...
    private static String printEpcisEvents(final List<EpcisEvent> events) {
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            sb.append("EPCIS event " + (i + 1) + PrintHelper.COLON);
    ...
    private static String printEpcisEvent(final EpcisEvent event) {
        ...
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Id: " + event.id);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Type: " + event.type);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Event time: " + event.eventTime);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Action: " + event.action);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Disposition: " + event.disposition);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Biz location: " + event.bizLocation);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Biz step: " + event.bizStep);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Read point: " + event.readPoint);
        sb.append(PrintHelper.NEW_LINE + PrintHelper.TAB + "Epc list: ");
